...
- 2026-08-22：确认前端空白问题由主插件 JAR 未包含 Vite CSS/JS 资源导致；已同步完整 `dist` 并重新生成主插件 JAR。
- 版本已推进到 `1.8.0`；版本检查改为 GitHub `releases/latest` API，仅使用正式版，不再读取分支 `pom.xml`。
- 管理员网页登录使用已注册账号的用户名/邮箱和密码，且该用户必须在服务器 `ops.json` 中（可通过 `/op <玩家名>` 添加）。
- 2026-08-22：代理插件已成功生成 `plugin-proxy/target/verifymc-proxy-1.8.0.jar`；主插件已重新同步完整 Vite `dist` 后生成 `plugin/target/verifymc-1.8.0.jar`，JAR 内包含 `static/glassx/index.html`、CSS 和 JS。
